...
$herdrAssignmentSender = Join-Path $root 'skills\multi-model-agent-orchestrator\scripts\send-herdr-worker-assignment.ps1'
...
if (-not (Test-Path -LiteralPath $herdrAssignmentSender -PathType Leaf)) { throw "Missing Herdr assignment sender: $herdrAssignmentSender" }
